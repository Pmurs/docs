...
* Run `systemctl set-environment LOAD_SNAPSHOT=0` or `export LOAD_SNAPSHOT=0` to disable automatic loading of snapshots (may require sudo).
* Start the docker validator containter or systemd service. If the environment variable change made above is not being detected by docker, you can tear down and recreate your docker container and pass the variable in manually by adding `-e "LOAD_SNAPSHOT=0"` to the run command. 
...
* Run `systemctl unset-environment LOAD_SNAPSHOT` to enable automatic loading of snapshots (may require sudo) OR tear down and recreate your container if you passed the variable manually.
